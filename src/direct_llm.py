...
import re
import random
#All vertex ai portion of the code is not used since we want to test the other parts
#and don't have access to vertex anyways
...
def dumb_classify_text(text):
    """
    Randomly assigns a category (a, b, c, d) and a sentiment (1, 0, -1).
    """
    # Randomly assign a category (a, b, c, d)
    category = random.choice(['a', 'b', 'c', 'd'])
    # Randomly assign a sentiment (1, 0, -1)
    sentiment = random.choice([1, 0, -1])
    return f"{category}, {sentiment}"
...
def process_dataframe(df, text_column):
    """
    Process the DataFrame by applying the dumb_classify_text function.
    """
    # Apply the dumb_classify_text function to each row
    df['classification'] = df[text_column].apply(dumb_classify_text)
...
if __name__ == "__main__":
    ...
    oversampled_data = random.choices(example_data, k=100)  # Oversample to 100 rows
    
    df = pd.DataFrame({
        'customer_text': oversampled_data
    })
